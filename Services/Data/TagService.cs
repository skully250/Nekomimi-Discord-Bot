...
namespace Nekomimi_Rewrite.Services
{
    ...
    public class TagService
    {
        ...
        public async Task ModifyTag(CommandContext context, SocketGuildUser user, string tagName, string content)
        {
            try
            {
                string tagContent;
                if (json.First(x => x.id == user.Id).tags.TryGetValue(tagName, out tagContent))
                {
                    json.First(x => x.id == user.Id).tags.TryUpdate(tagName, content, tagContent);
                    await context.Channel.SendMessageAsync($"Tag {tagName} was successfully updated");
                } else
                {
                    await context.Channel.SendMessageAsync($"Tag {tagName} was not updated successfully");
                }
                saveTags();
            } catch (Exception e)
            {
